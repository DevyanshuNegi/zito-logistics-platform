...
export default function CustomerInvoicesPage() {
  ...
  async function loadInvoices() {
    ...
    try {
      const response = await api.get<InvoicesResponse>('/customer/invoices');
  ...
  return (
    <div className="space-y-6">
      <div className="grid gap-4 md:grid-cols-3">
        <StatCard label="Invoices" value={String(invoices.length)} helper="All invoices available to your account." />
        <StatCard label="Outstanding" value={formatMoney(outstandingTotal)} helper="Remaining balance across unpaid invoices." tone="info" />
        <StatCard label="Paid to date" value={formatMoney(paidTotal)} helper="Amount already settled on issued invoices." tone="success" />
      </div>
      ...
      <SurfaceCard
        ...
        description="Issued invoices generated from completed transport, courier, and warehouse work."
      >
        {loading ? (
          <Spinner />
        ) : (
