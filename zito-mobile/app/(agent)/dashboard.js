    ...
    <SafeAreaView style={styles.root}>
      ...
      >
        ...
        <View style={styles.kpiGrid}>
          <KpiTile label="Open Opportunities" value={String(opportunities.length)} tone={colors.primary} />
          <KpiTile label="Fixed Price" value={String(fixedPriceCount)} tone={colors.info} />
          <KpiTile label="Open Bid" value={String(openBidCount)} tone={colors.warning} />
          <KpiTile
        ...
        <SectionCard title="Partner profile" subtitle="Marketplace verification, service areas, and commercial setup.">
          <View style={styles.profileHead}>
            <View style={{ flex: 1 }}>
              <Text style={styles.profileName}>{profile.companyName || 'Agent partner profile'}</Text>
              <Text style={styles.meta}>
                Commission {Number(profile.commissionRatePct || 0)}% · Service fee KES{' '}
                {Number(profile.serviceFeeFlat || 0).toLocaleString()}
              </Text>
            </View>
            <StatusBadge status={String(profile.verificationStatus || 'pending_review').toLowerCase()} />
          </View>
        ...
        <SectionCard title="Next loads" subtitle="The latest matching marketplace opportunities available to this agent profile.">
          {opportunities.length === 0 ? (
            <Text style={styles.copy}>No opportunities are open yet. Refresh after dispatch publishes new partner loads.</Text>
          ) : (
            opportunities.slice(0, 3).map((item) => (
              <View key={item.bookingId} style={styles.loadCard}>
                <View style={styles.loadTop}>
                  <Text style={styles.loadTitle}>{item.booking?.reference || item.bookingReference || 'Marketplace load'}</Text>
                  <StatusBadge status={String(item.status || 'open').toLowerCase()} />
                </View>
                <Text style={styles.meta}>
                  {item.serviceType} · {item.pricingModel?.replace('_', ' ')} ·{' '}
                  {(item.serviceAreaHints || []).join(', ') || 'Coverage by route'}
                </Text>
              </View>
            ))
          )}
...
const styles = StyleSheet.create({
  ...
  profileHead: { flexDirection: 'row', gap: 10, justifyContent: 'space-between', alignItems: 'flex-start' },
  ...
  loadTop: { flexDirection: 'row', justifyContent: 'space-between', gap: 10, marginBottom: 10 },
